...
def main():
    ...
    client_train_loader, test_loader, data_size_per_client = get_data_loaders(num_clients, batch_size, real_wd=True,
                                                                              weighted=weighted1)
    ...
    for r in range(num_rounds):
        ...
        print('%d-th round' % r)
        print('average train loss %0.3g | test.py loss %0.3g | test.py acc: %0.3f' % (
        ...
        name = "log_bandit_lr" + str(lr) + "_decay" + str(decay_factor) + "_alpha" + str(alpha) + "_C" + str(
            num_clients) + "_S" + str(num_selected) + "_Nbatch" + str(num_batch) + "_weighted" + str(
            weighted1) + "_DataSize" + str(ds) + "_CAlpha" + str(confident_alpha) + ".mat"

        sio.savemat(name, {'reward_client': reward_client, 'acc_test': acc_test, 'T_pull': T_pull_lst,
                           'client_idx': client_idx_lst})
...
def server_aggregate(global_model, client_models, data_size_weights, client_idx):
    ...
    for k in global_dict.keys():
        global_dict[k] = torch.stack(
            [data_size_weights[client_idx[i]] * client_models[i].state_dict()[k].float() for i in
             range(len(client_models))], 0).sum(0) + sum([data_size_weights[idx] for idx in non_sampled_client_idx]) * \
                         global_model.state_dict()[k].float()
