...
def client_train_1(model, train_loader, criterion, optimizer, num_epochs, num_batches):
    ...
    # 计算权重更新量
    pseudo_grads = torch.autograd.grad(loss,
                                       model.module.params() if torch.cuda.device_count() > 1 else model.params(),
                                       create_graph=True)
...
optimizer_model = torch.optim.SGD(global_model.module.params() if torch.cuda.device_count() > 1 else global_model.params(),
                                  lr, momentum=momentum, nesterov=nesterov, weight_decay=weight_decay)

# 模拟多个客户端
client_models = [build_model(dataset) for _ in range(num_clients)]

client_models = [model.to(device) for model in client_models]

client_optimizers = [torch.optim.SGD(model.module.params() if torch.cuda.device_count() > 1 else model.params(),
                                     lr, momentum=momentum, nesterov=nesterov, weight_decay=weight_decay)
                                    for model in client_models]

# 初始化元学习网络
meta_net = MLPHC(hidden_size=meta_net_hidden_size,
                 num_layers=meta_net_num_layers,
                 k=num_selected,
                 hard=True)

# 检查是否有多张显卡，并使用 DataParallel
if torch.cuda.device_count() > 1:
    meta_net = torch.nn.DataParallel(meta_net)

meta_net = meta_net.to(device)  # 将模型转移到 GPU 上

# 初始化优化器
meta_optimizer = torch.optim.Adam(meta_net.module.parameters() if torch.cuda.device_count() > 1 else meta_net.parameters(),
                                  lr=meta_lr,
...
for round in range(num_rounds):
    ...
    # 本次传输成功与否，此处设置为失败的概率
    features = torch.cat([client_losses_tensor, torch.tensor(prob_vector).view(-1, 1).to(device)], dim=1)

    if torch.cuda.device_count() > 1:
        pseudo_weight = meta_net.module(features)
    else:
        pseudo_weight = meta_net(features)

    # features = torch.tensor(list(zip(client_losses_tensor, prob_vector)), requires_grad=True).float().to(device)

    # pseudo_weight = meta_net(features.data)
    ...
    # 加权平均
    success_client = 0
    for grads, weight, isSuccess in zip(grads_list, pseudo_weight[0], client_com_prob_r_tensor_clip[0]):
    ...
    for i in range(len(aggregated_grads)):
        aggregated_grads[i] /= success_client

    # 在多GPU环境下，更新全局模型的参数
    if torch.cuda.device_count() > 1:
        pseudo_net.module.update_params(lr_inner=lr, source_params=aggregated_grads)
    else:
    ...
    # 检查是否使用 DataParallel
    if isinstance(meta_net, torch.nn.DataParallel):
        # 使用 module 属性访问实际模型对象
        weights = meta_net.module.output_layer.weight
    else:
        weights = meta_net.output_layer.weight

    # 打印权重的前 5 个值
    print('meta_net.output_layer.weight after: ', weights[0, 0:5].cpu())
    ...
    # test_comm_probs = torch.arange(0.1 * num_clients + 0.1, 0.1, 0.1).to(device)
    # test_comm_probs = torch.tensor(np.arange(1, 0, -0.1)).float().to(device)
    test_comm_probs = torch.rand(num_clients).float().to(device)
    test_train_losses = torch.arange(num_clients).float().to(device)  # 随机生成通信成功概率
    ...
    # torch.save(meta_net.state_dict(), f'/home/xm/code/meta_model/mlph_model_com_LN_hard_S{num_selected}_N{num_clients}_BS{batch_size}_{time_str}.pth')
    # torch.save(global_model.state_dict(), f'/home/xm/code/meta_model/global_model_com_LN_hard_S{num_selected}_N{num_clients}_BS{batch_size}_{time_str}.pth')