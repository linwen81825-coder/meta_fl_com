...
#prob_vector = prob_vector * 0.7 + 0.3
...
for round in range(num_rounds):
    ...
    print('client_loss: ', client_losses_tensor)
    #client_com_prob_tensor = torch.tensor(prob_vector).view(-1, 1).to(device)
    # 本次传输成功与否，此处设置为失败的概率

    pseudo_weight = meta_net(prob_vector.data)
    ...
    meta_loss = criterion(meta_outputs, meta_labels.long()) #+ torch.exp(torch.tensor(1 - success_rate)).to(device)#/ success_rate
    ...
    torch.save(meta_net.state_dict(), f'/home/xm/code/meta_model/mlph_model_com_hard_sparse_{time_str}.pth')
    torch.save(global_model.state_dict(), f'/home/xm/code/meta_model/global_model_com_hard_sparse_{time_str}.pth')
