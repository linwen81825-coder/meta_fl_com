...
def build_model(dataset, layers=5, widen_factor=1, droprate=0):
...
def server_aggregate1(global_model, client_models, data_size_weights):
    ...
    for k in global_dict.keys():
        ...
        for i, client_model in enumerate(client_models):
            ...
            global_dict[k] += data_size_weights[i] * client_state_dict[k].float()

    # Normalize the aggregated parameters by the sum of weights to ensure correct averaging
    total_weight = sum(data_size_weights)
    for k in global_dict.keys():
...
train_dataloaders, test_dataloader, meta_dataloader = get_data_loaders_new(num_clients, batch_size, meta_bs, meta_sample_number, isnoise=False)
...
# 初始化元学习网络
meta_net = MLPHC(hidden_size=meta_net_hidden_size, num_layers=meta_net_num_layers, k=num_selected, hard=True).to(device)
...
for round in range(num_rounds):
    ...
    torch.save(meta_net.state_dict(), f'/home/xm/code/meta_model/mlph_model_s_COM_LN_hard_S{num_selected}_N{num_clients}_BS{batch_size}_{time_str}.pth')
    torch.save(global_model.state_dict(), f'/home/xm/code/meta_model/global_model_s_COM_LN_hard_S{num_selected}_N{num_clients}_BS{batch_size}_{time_str}.pth')
