...
def get_mnist():
    ...
    data_train = datasets.FashionMNIST('data', train=True, transform=transform, download=True)
    data_test = datasets.FashionMNIST('data', train=False, transform=transform, download=True)
...
def split_image_data_realwd(data, labels, n_clients=100, verbose=True):
    ...
    print('total_partition:', total_partition)
    ...
    for i in range(n_clients):
        ...
        while n > 0:
            ...
            if len(class_partition_split[class_]) > 0:
                if class_ in {1, 5, 7} and len(class_partition_split[class_]) > fix_thres:
    ...
    def print_split(clients_split):
        print("Data split:")
        cls = [0 for i in range(10)]
        for i, client in enumerate(clients_split):
            split = np.sum(client[1].reshape(1, -1) == np.arange(n_labels).reshape(-1, 1), axis=1)
            print(" - Client {}: {}".format(i, split))
            
            for idx, num in enumerate(split):
                cls[idx] += num
        print(cls)
...
def get_data_loaders(nclients, batch_size, real_wd, ratio, weighted=True, dataset='cifar'):
    ...
    if real_wd:
        split = split_image_data_realwd(x_train, y_train, n_clients=nclients, verbose=True)
    else:
        split = split_image_data(x_train, y_train, n_clients=nclients,
                                 classes_per_client=classes_pc, verbose=True)
    ...
    print(data_size_per_client)

    return client_loaders, test_loader, data_size_per_client


# def main():
#     loaders = get_data_loaders(64, 128, True, 0.1, True, dataset='mnist')
#     print('test')
