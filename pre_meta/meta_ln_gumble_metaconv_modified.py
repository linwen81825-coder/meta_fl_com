...
def server_aggregate(global_model, client_models, num_selected):
...
num_rounds = 1	# num of communication rounds
...
# meta_optimizer = torch.optim.Adam(vnet.parameters(), lr=meta_lr, weight_decay=meta_weight_decay)
...
for r in range(num_rounds):
    ...
    for m in range(M):
        ...
        for i in range(num_clients):  
            cost_set_v[i] = cal_loss(client_train_loader_next[i], pseudo_net)

        # receive training loss of each client
        # reshape loss
        cost_set_v = torch.reshape(cost_set_v, (len(cost_set_v), 1))

        #cost_set_v = loss_normalize(cost_set_v, 1).to(device)
        ...
        pseudo_net.update_params(lr_inner=meta_lr, source_params=grads_avg)
        ...
        print('test_out:', vnet(x))

        test_loss, acc = test(pseudo_net, test_loader)
