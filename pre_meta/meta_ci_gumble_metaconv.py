...
from dataSplitTii import get_data_loaders
...
class VNet(MetaModule):
    ...
    def forward(self, x):
        ...
        print("=====hhh=====")
        print(x.shape) # 128*1
        print(self.subset_sample(torch.transpose(x, 0, 1)).shape)
        print("==========")
...
# load dataset
# client_train_loader, test_loader, data_size_per_client = get_data_loaders(num_clients, batch_size, real_wd=True, weighted=weighted1, classes_pc=10, verbose=True)
client_train_loader, test_loader, data_size_per_client = get_data_loaders(num_clients, batch_size, True, ratio=0.4, weighted=weighted1, dataset='cifar')
...
for r in range(num_rounds):
    ...
    name = "./mat/0816_metaCI_gumble_lr"+str(lr)+"_C"+str(num_clients)+"_S"+str(num_selected)+"_Nbatch"+str(real_num_batch)+".mat"
