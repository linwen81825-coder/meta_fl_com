...
def main():
    ...
    global_model = WideResNet(layers, dataset == 'cifar10' and 10 or 100,
                       widen_factor, dropRate=droprate).to(device)
    client_models = [WideResNet(layers, dataset == 'cifar10' and 10 or 100,
                       widen_factor, dropRate=droprate).to(device) for _ in range(num_selected)]
    ...
    opt_lst = [torch.optim.SGD(global_model.params(), lr,
                                  momentum=momentum, nesterov=nesterov,
    ...
    for r in range(num_rounds):
        ...
        name = "log_wideresnet_random_lr"+str(lr)+"_decay"+str(decay_factor)+"_alpha"+str(alpha)+"_C"+str(num_clients)+"_S"+str(num_selected)+"_Nbatch"+str(num_batch)+"_weighted"+str(weighted1)+"_DataSize"+str(ds)+"_CAlpha"+str(confident_alpha)+".mat"
